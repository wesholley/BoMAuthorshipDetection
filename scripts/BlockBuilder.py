...
# Builds blocks based upon files of tagged output    
...
#             print "Curblock:", len(curBlock.split('\n'))
            ...
            if (blockLineCount == 199):     # end of the current block (200 words)
                ...
                saveBlockToFile(blocks_directory, filename, blocksMade + 1, curBlock)
                curBlock = ""
...
def saveBlockToFile(blocks_directory, filename, blockNumber, curBlock):
    ...
    file_directory = blocks_directory + "/" + filename + "_block_" + str(blockNumber) + ".txt"
    block_file = open(file_directory, 'w+')
    block_file.write(curBlock)
    block_file.close()
...
if __name__ == '__main__':
    """ load the files """
    INPUT_PATH = "../TaggedOutput/"
    OUTPUT_PATH = "../200WordBlocks/"
